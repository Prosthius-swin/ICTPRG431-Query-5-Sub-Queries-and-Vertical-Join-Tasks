...
WHERE BIRTHCOUNTRY IN (SELECT BIRTHCOUNTRY 
                      FROM ACTOR9468
                      WHERE BIRTHCOUNTRY = 'Australia'
                      OR
                      BIRTHCOUNTRY =  'England')

-- 4
SELECT M.RATINGCODE, AVG(RUNTIME) 'Average Length'
FROM MOVIE9468 M
GROUP BY M.RATINGCODE
HAVING AVG(RUNTIME) > (SELECT AVG(RUNTIME)
                      FROM MOVIE9468)

-- 5
SELECT A.FULLNAME, 'Limited' AS Castings
FROM MOVIE9468 M
INNER JOIN CASTING9468 C
ON M.MOVIENO = C.MOVIENO
INNER JOIN ACTOR9468 A
...
GROUP BY A.FULLNAME
HAVING COUNT(C.MOVIENO) < 4
UNION
SELECT A.FULLNAME, 'Significant' AS Castings
FROM MOVIE9468 M
INNER JOIN CASTING9468 C
ON M.MOVIENO = C.MOVIENO
INNER JOIN ACTOR9468 A
ON A.ACTORNO = C.ACTORNO
GROUP BY A.FULLNAME
HAVING COUNT(C.MOVIENO) BETWEEN 4 AND 8
UNION
SELECT A.FULLNAME, 'Prolific' AS Castings
FROM MOVIE9468 M
INNER JOIN CASTING9468 C
ON M.MOVIENO = C.MOVIENO
INNER JOIN ACTOR9468 A
ON A.ACTORNO = C.ACTORNO
GROUP BY A.FULLNAME
HAVING COUNT(C.MOVIENO) >= 9

-- 6
INSERT INTO COLOURTYPE9468 (colourcode, colourname) VALUES ('X', 'Black & White Movie');

SELECT R.SHORTDESC
FROM RATING9468 R
UNION ALL
SELECT C.COLOURNAME
FROM COLOURTYPE9468 C

DELETE FROM COLOURTYPE9468 WHERE colourcode = 'X';
...
ORDER BY TITLE ASC, RELYEAR ASC
